...
<!-- --------------------------------------------------------------메인------------------------------------------------------------ -->		
			...
			<div class="col-sm-9">
				<h2 style = "font-weight: bold;">오프라인 스터디</h2>
				<hr align="left" width="300px" style="background-color: #c47100; height:1px;" />
				<br>
				<h4>${menuName}</h4>
...
<!-- --------------------------------------------------------------페이지 번호------------------------------------------------------------ -->
				...
				<nav class="container">
					...
					<li 
						...
						<a class="page-link " href="<%=request.getContextPath()%>/studymenu/offAllSearchList?pageNum=${startPage - bottomLine}">이전</a></li>
						
					<c:forEach var="i" begin="${ startPage }" end="${endPage}">
						<li class='page-item <c:if test = "${i == pageInt}" >  active active2</c:if>'>
						<a class="page-link" href="<%=request.getContextPath()%>/studymenu/offAllSearchList?pageNum=${i}&searchData=${searchData}&part=${part}">${i}</a></li>
					
					</c:forEach>
						<li class='page-item <c:if test = "${a_endPage >= a_maxPage}"> disabled </c:if>'>
						<a class="page-link" href="<%=request.getContextPath()%>/studymenu/offAllSearchList?pageNum=${startPage + bottomLine}">다음</a></li>
					</ul>
				</nav>
				</c:if>
				...
				<nav class="container">
					...
					<li 
						class='page-item <c:if test="${startPage <= bottomLine}"> disabled </c:if>'>
						<a class="page-link " href="<%=request.getContextPath()%>/studymenu/offSearchList?pageNum=${startPage - bottomLine}">이전</a></li>
						
					<c:forEach var="i" begin="${ startPage }" end="${endPage}">
						<li class='page-item <c:if test = "${i == pageInt}" > active active2 </c:if>' >
						<a class="page-link " href="<%=request.getContextPath()%>/studymenu/offSearchList?pageNum=${i}&searchData=${searchData}&part=${part}">${i}</a></li>
					
					</c:forEach>
						<li class='page-item <c:if test = "${endPage >= maxPage}"> disabled </c:if>'>
						<a class="page-link" href="<%=request.getContextPath()%>/studymenu/offSearchList?pageNum=${startPage + bottomLine}">다음</a></li>
					</ul>
				</nav>
				</c:if>
			</div>
